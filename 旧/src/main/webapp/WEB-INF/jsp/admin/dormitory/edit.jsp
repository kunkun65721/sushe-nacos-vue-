            ...
            <div class="flex items-center gap-sm mb-lg">
                ...
                <div>
                    <h2 class="font-h2 text-h2 text-on-surface flex items-center gap-sm">
                        <span class="material-symbols-outlined text-primary">edit</span> 编辑宿舍
                    </h2>
                    <p class="font-body-md text-body-md text-on-surface-variant mt-xs">更新宿舍信息。</p>
                </div>
            </div>
            <div class="bg-surface-container-lowest border border-outline-variant rounded-xl p-md shadow-soft">
                <form action="${pageContext.request.contextPath}/admin/dormitory/edit" method="post" id="dormitoryForm">
                    ...
                    <div class="mb-md">
                        ...
                        <input type="text" id="dormitoryNumber" name="dormitoryNumber" value="${dormitory.dormitoryNumber}" required
                    ...
                    <div class="mb-md">
                        ...
                        <select id="dormitoryType" name="dormitoryType" required class="form-select">
                            <option value="4人间" ${dormitory.dormitoryType == '4人间' ? 'selected' : ''}>4人间</option>
                            <option value="6人间" ${dormitory.dormitoryType == '6人间' ? 'selected' : ''}>6人间</option>
                            <option value="8人间" ${dormitory.dormitoryType == '8人间' ? 'selected' : ''}>8人间</option>
                            <option value="其他" ${dormitory.dormitoryType == '其他' ? 'selected' : ''}>其他</option>
                        </select>
                    </div>
                    <div class="grid grid-cols-2 gap-md mb-md">
                        <div>
                            ...
                            <input type="number" id="capacity" name="capacity" value="${dormitory.capacity}" required
                                   class="w-full bg-surface-container-lowest border border-outline-variant rounded-lg px-sm py-xs font-body-md text-body-md text-on-surface focus:border-primary focus:ring-1 focus:ring-primary outline-none transition-colors">
                        </div>
                        <div>
                            <label for="currentOccupancy" class="block font-label-caps text-label-caps text-on-surface-variant mb-xs">
                                <span class="material-symbols-outlined text-[14px] align-middle">person_check</span> 当前入住
                            </label>
                            <input type="number" id="currentOccupancy" name="currentOccupancy" value="${dormitory.currentOccupancy}" required
                                   class="w-full bg-surface-container-lowest border border-outline-variant rounded-lg px-sm py-xs font-body-md text-body-md text-on-surface focus:border-primary focus:ring-1 focus:ring-primary outline-none transition-colors">
                        </div>
                    </div>
                    <div class="mb-md">
                        ...
                        <input type="text" id="building" name="building" value="${dormitory.building}" required
                               class="w-full bg-surface-container-lowest border border-outline-variant rounded-lg px-sm py-xs font-body-md text-body-md text-on-surface focus:border-primary focus:ring-1 focus:ring-primary outline-none transition-colors">
                    </div>
                    <div class="mb-lg">
                        ...
                        <select id="status" name="status" required class="form-select">
                            <option value="1" ${dormitory.status == 1 ? 'selected' : ''}>启用</option>
                            <option value="0" ${dormitory.status == 0 ? 'selected' : ''}>禁用</option>
                        </select>
                    </div>
