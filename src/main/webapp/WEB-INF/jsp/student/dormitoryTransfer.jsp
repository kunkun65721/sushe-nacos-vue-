        ...
        <div class="p-margin max-w-[800px] w-full mx-auto">
            <div class="mb-lg">
                <h2 class="font-h2 text-h2 text-on-surface flex items-center gap-sm">
                    <span class="material-symbols-outlined text-primary">swap_horiz</span> 宿舍调换申请
                </h2>
                <p class="font-body-md text-body-md text-on-surface-variant mt-xs">提交宿舍调换申请并查看申请进度。</p>
            </div>
            ...
            <c:choose>
                <c:when test="${allocation == null}">
                    <div class="bg-surface-container-lowest border border-outline-variant rounded-xl p-xl shadow-soft text-center">
                        ...
                        <h3 class="font-h3 text-h3 text-on-surface mb-sm">无法申请调换</h3>
                        <p class="font-body-md text-body-md text-on-surface-variant mb-lg">您尚未分配宿舍，请先等待管理员分配宿舍后再申请调换。</p>
                ...
                <c:otherwise>
                    ...
                    <div class="bg-surface-container-lowest border border-outline-variant rounded-xl shadow-soft overflow-hidden mb-gutter">
                        <div class="p-md border-b border-outline-variant">
                            <h3 class="font-h3 text-h3 text-on-surface flex items-center gap-sm">
                                <span class="material-symbols-outlined text-primary">edit_note</span> 提交调换申请
                            </h3>
                        </div>
                        <div class="p-md">
                            <form id="transferForm" action="${pageContext.request.contextPath}/student/transfer" method="post" enctype="multipart/form-data">
                                <div class="mb-md">
                                    <label class="block font-label-caps text-label-caps text-on-surface-variant mb-xs">调换原因 <span class="text-error">*</span></label>
                                    <textarea name="reason" rows="4" maxlength="500" required
                                        ...
                                        placeholder="请详细说明调换宿舍的原因..."></textarea>
                                    <p class="font-body-sm text-[12px] text-on-surface-variant mt-xs">最多500字</p>
                                </div>
                                <div class="mb-md">
                                    <label class="block font-label-caps text-label-caps text-on-surface-variant mb-xs">意向宿舍（可选）</label>
                                    <select name="targetDormitoryId"
                                        ...
                                        <option value="">-- 暂不指定（由管理员分配）--</option>
                                        <c:forEach items="${dormitoryList}" var="d">
                                            <c:if test="${d.status == 1 && d.id != allocation.dormitoryId && d.currentOccupancy < d.capacity}">
                                                <option value="${d.id}">${d.building} ${d.dormitoryNumber}（${d.dormitoryType}，空余${d.capacity - d.currentOccupancy}床位）</option>
                                            </c:if>
                                        </c:forEach>
                                    </select>
                                </div>
                                <div class="mb-md">
                                    <label class="block font-label-caps text-label-caps text-on-surface-variant mb-xs">附件（可选）</label>
                                    <div class="border border-dashed border-outline-variant rounded-lg p-md text-center bg-surface-container-lowest">
                                        <span class="material-symbols-outlined text-[40px] text-outline mb-sm block">upload_file</span>
                                        <p class="font-body-md text-body-md text-on-surface-variant mb-sm">拖拽文件到此处或点击上传</p>
                                        <p class="font-body-sm text-[12px] text-on-surface-variant mb-sm">支持图片、PDF等，单文件不超过10MB</p>
                    ...
                    <div class="bg-surface-container-lowest border border-outline-variant rounded-xl shadow-soft overflow-hidden">
                        <div class="p-md border-b border-outline-variant">
                            <h3 class="font-h3 text-h3 text-on-surface flex items-center gap-sm">
                                <span class="material-symbols-outlined text-primary">history</span> 申请记录
                            </h3>
                        </div>
                        <c:choose>
                            <c:when test="${empty transferList}">
                                <div class="p-xl text-center">
                                    ...
                                    <p class="font-body-md text-body-md text-on-surface-variant">暂无申请记录</p>
                                </div>
                            </c:when>
                            <c:otherwise>
                                    ...
                                    <table class="w-full">
                                        <thead>
                                            <tr class="border-b border-outline-variant">
                                                <th class="font-label-caps text-label-caps text-on-surface-variant text-left p-sm">申请时间</th>
                                                <th class="font-label-caps text-label-caps text-on-surface-variant text-left p-sm">意向宿舍</th>
                                                <th class="font-label-caps text-label-caps text-on-surface-variant text-left p-sm">理由</th>
                                                <th class="font-label-caps text-label-caps text-on-surface-variant text-left p-sm">附件</th>
                                                <th class="font-label-caps text-label-caps text-on-surface-variant text-left p-sm">状态</th>
                                                <th class="font-label-caps text-label-caps text-on-surface-variant text-left p-sm">审核意见</th>
                                            </tr>
                                        </thead>
                                        <tbody>
                                            <c:forEach items="${transferList}" var="t">
                                                <tr class="border-b border-outline-variant hover:bg-surface-container transition-colors">
                                                    <td class="p-sm font-body-md text-body-md text-on-surface">${t.applyTime}</td>
                                                    ...
                                                    <td class="p-sm font-body-md text-body-md text-on-surface max-w-[200px] truncate" title="${t.reason}">${t.reason}</td>
                                                    <td class="p-sm">
                                                        <c:if test="${not empty t.attachment}">
                                                            <a href="${pageContext.request.contextPath}/${t.attachment}" target="_blank" class="inline-flex items-center gap-xs text-primary hover:text-on-primary-fixed-variant font-label-caps text-label-caps">
                                                                <span class="material-symbols-outlined text-[18px]">attach_file</span>查看
                                                            </a>
                                                        </c:if>
                                                        <c:if test="${empty t.attachment}"><span class="text-on-surface-variant">-</span></c:if>
                                                    </td>
                                                    <td class="p-sm">
                                                        <c:choose>
                                                    ...
                                                    <td class="p-sm font-body-md text-body-md text-on-surface max-w-[200px] truncate" title="${t.adminComment}">
                                                        <c:choose>
                                                            <c:when test="${not empty t.adminComment}">${t.adminComment}</c:when>
